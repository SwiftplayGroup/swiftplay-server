...
export default class User {
  ...
  verifyPermission(
    ...
    requiredPermissionLevel: 0 | 1 | 2,
  ) {
    const permissionTree = permissionName.split(".");
    type SelectedPermission = { [key: string]: number | SelectedPermission };
    let selectedDefaultPermissionObject: SelectedPermission =
      User.defaultPermissions;
    type SelectedPermissionOverride = {
      [key: string]: number | SelectedPermissionOverride | undefined;
    };
    let selectedOverridePermissionObject:
      | SelectedPermissionOverride
      | undefined = this.permissionOverrides;
    ...
    for (const permission of permissionTree) {
      if (typeof selectedDefaultPermissionObject[permission] == "number") {
        ...
        selectedDefaultPermissionObject =
          selectedDefaultPermissionObject[permission];
      }

      if (selectedOverridePermissionObject?.[permission]) {
        if (typeof selectedOverridePermissionObject[permission] == "number") {
          overridePermissionLevel =
            selectedOverridePermissionObject[permission];
        } else {
          selectedOverridePermissionObject =
            selectedOverridePermissionObject[permission];
        }
      } else {
    ...
    if (
      typeof overridePermissionLevel === "number"
        ? overridePermissionLevel < requiredPermissionLevel
        : defaultPermissionLevel < requiredPermissionLevel
    ) {
